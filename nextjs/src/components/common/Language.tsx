...
const Language: React.FC<{ customerClassName?: string }> = ({
  customerClassName,
}) => {
  ...
  return (
    <div
      className={`dropdown dropdown-top  ${customerClassName} max-sm:text-white max-sm:bg-btnbg`}
    >
      <label tabIndex={0} className="flex space-x-1 text-xl cursor-pointer ">
        ...
        <div className="max-sm:text-sm flex-1 flex items-center justify-center">
          {' '}
      ...
      <ul
        tabIndex={0}
        className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-28 md:w-36 "
      >
        {languages.map((item) => (
          <li key={item.value}>
            <a
